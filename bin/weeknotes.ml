            ...
            El.ul
              ...
              [
                El.li
                  [
                    El.span
                      [ El.a ~at:[ At.href url ] [ El.txt (Page.title page) ] ];
                    El.span [ El.txt date ];
                  ];
              ];
          ])
...
let render_page site sec previous_page page next_page =
  ...
  let article_head =
    match is_old_weeknotes with
    | true -> [ El.h1 ~at:[ At.class' "title" ] [ El.txt raw_title ] ]
    | false ->
        [
          El.h1
            ~at:[ At.class' "title" ]
            [ El.txt (Printf.sprintf "Weeknotes: %s" raw_title) ];
          El.p ~at:[ At.class' "date" ] [ El.txt date ];
        ]
  in

  let tags =
    match Page.tags page with
    | [] -> []
    | tags ->
        let items =
          List.map
            (fun tag ->
              let term_for_url =
                String.map (fun c -> match c with ' ' -> '-' | x -> x) tag
              in
              El.a
                ~at:[ At.href (Printf.sprintf "/tags/%s/" term_for_url) ]
                [ El.txt tag ])
            tags
        in
