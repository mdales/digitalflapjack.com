...
let render_pub_body show_abstract sec_opt page =
  let authorslist =
    match Page.get_key_as_yaml page "authors" with
    | Some yaml -> (
        match yaml with
        | `A lst ->
            List.concat_map
  ...
  let pubtype =
    match Page.get_key_as_string page "type" with
  ...
  let conf =
    ...
    | conf -> (
        ...
        | Some url ->
            [ El.txt " in "; El.a ~at:[ At.href url ] [ El.txt title ] ]
        | None -> [ El.txt title ])
  in
  ...
  let opt_link key label href_fn =
    ...
    | Some url ->
        [ El.txt " | "; El.a ~at:[ At.href (href_fn url) ] [ El.txt label ] ]
    | None -> []
  in
...
let render_section site sec =
  ...
  let paperslist =
    List.concat_map
      (fun page -> render_pub_body false (Some sec) page)
      (Section.pages sec)
  in
              ...
              [
                ...
                El.div
                  ...
                  [
                    El.div
                      ~at:[ At.class' "article" ]
                      [ El.article (render_pub_body true None page); nav_links ];
                  ];
              ];
            El.div ~at:[ At.id "footer" ] [];
            El.div
