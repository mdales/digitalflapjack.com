...
let render_section site sec =
  ...
  let projects = List.map (fun page ->
    ...
    | Some filename -> (
      ...
      | ".svg" -> (
        El.div ~at:[
          ...
          At.style ("background-image: url(" ^ urlbase ^ "thumbnail.svg");
        ] []
      )
      | _ -> (
        El.img ~at:[
          ...
          At.v "srcset" (urlbase ^ "thumbnail@2x.jpg 2x, " ^ urlbase ^ "thumbnail.jpg 1x")
        ] ()
      )
    )
    | None -> (
      El.div ~at:[At.class' "projecticon"] [];
    )
    in

    let prose = match (Page.synopsis page) with
    | Some prose -> prose
    | None -> ""
    in
    ...
    let inner = El.div [
      ...
      El.p ~at:[At.class' "projectdate"] [El.txt (Renderer.ptime_to_str (Page.date page))];
      El.p [El.txt prose]
    ] in

    let wrapped_inner = match (has_source || has_content) with
    | true -> (
      let url = match src with
      | Some url -> url
